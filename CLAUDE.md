...
| 取り込んだ旅を**送り返す** | **無料**(初回共有から1年) | 止めると A→B→A の往復が切れる |
| 自分の旅を**はじめて共有する** | **Pro** | 唯一の課金点 |
| 一度共有した旅を**再送する** | **無料**(解約後も・初回共有から1年) | 旅行中に期限切れで送れなくなるのを防ぐ |

**無期限にしない**(`FREE_RESHARE_WINDOW_MS`)。無期限だと**一円も払わずに
使い放題にできた** ── 誰かから1つ受け取れば `imported` で送り放題になるので、
名前と日付と予定を入れ替えて「共有用の器」として永久に使い回せる。
サーバーもアカウントも持たない以上、「別の旅に作り変えたか」は見分けられない。
**90日ではなく1年**なのは、海外旅行や連休の旅が3〜6か月前から計画されるため。
短くすると、旅の準備中に正当な送り返しを止めてしまう。
