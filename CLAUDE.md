...
- 収益モデル: **自動更新サブスクのみ**(月額¥500)。**広告は入れない**
  (価格と無料/Proの線引き: [docs/pricing.md](docs/pricing.md))
...
### 収益:自動更新サブスクのみ(月額¥500)

詳細は [docs/pricing.md](docs/pricing.md)。設計に効く点だけここに。

- **無料でも旅を1つ、機能制限なしで使える。** 課金しないと何もできないのは
  Guideline 3.1.2 / 4.2 のリジェクト定番であり、そもそも誰も試せない
- **Pro の軸は「旅をいくつでも同時に持てる」。** サブスクには
  **継続的な価値(ongoing value)** が要る(3.1.2)。一度きりの機能を並べると弾かれる
- **共有は絶対に有料にしない。** 送られた相手が読めなければ機能が成立しない
- **解約してもデータをロックしない。** 既存の旅は読めるし編集もできる。
  できなくなるのは「2つめ以降を新しく作ること」だけ。
