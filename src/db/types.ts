...
export type Trip = SyncFields & {
  ...
  /**
   * **この端末が、この旅を送れるようになった時刻。** 無料期間(1年)の起点。
   *
   * `sharedAt` と分けてあるのは、**両者の持ち主が違う**から:
   * - `sharedAt` = この旅が世界で最初に共有された時刻。ファイルに乗って伝わる
   * - `shareWindowFrom` = **この端末**の話。自分が初めて送った時刻、
   *   または**受け取った時刻**。ファイルの値は信用せず、受け取った側で振り直す
   *
   * ⚠️ 分けないと**往復が切れる。** 起点を `sharedAt` にすると、
   * 1年以上前に共有された旅を受け取った人が**最初から送り返せない**。
   * 「取り込んだ旅を送り返す → 無料」は、崩すと共有そのものが死ぬ行
   * (CLAUDE.md の4行の表)。監査で見つかった。
   *
   * 一度立てたら**受け取り直しても更新しない**(`apply.ts` は `?? Date.now()`)。
   * 更新すると A↔B で往復し続けるだけで無期限になる。
   */
  shareWindowFrom?: number;
