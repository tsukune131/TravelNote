...
ようこそ画面 → 旅の作成 → 空状態のチップ → 予定5件の連続追加 →
詳細シート(時刻・所要時間・リンク)→ 今日の Day で現在時刻ライン →
地図アプリの初回選択 → 一覧へ戻る → 再読み込みで「今日」へ直行、まで通す。
...
| `dur :: <予定名> :: <分>` | 所要時間(15/30/45/60/90/120/180/240)。**「間に合わない警告」を出すシナリオにはこれが要る**(終わる時刻が動かないと空きが縮まらない) |
| `click / fill / press :: <selector> [:: <値>]` | Playwright セレクタで操作 |
| `longpress :: <selector>` | 長押し → 予定のアクションメニュー(リフロー・上下移動・別の日へ・複製) |
| `swipe :: <selector> :: right\|left` | 右=行った / 左=削除。72px 超で発火 |
...
- **dev サーバを殺し損ねるとポートを掴んだまま残る。** `shell: true` で起動しているので
  ...
  **かつ `spawnSync` で**。非同期で投げた直後に `process.exit()` すると実行前に消える。
  ...
  EPIPE で死んでも残らない
- **`reset` の直後はようこそ画面が出る**(初回起動と同じ状態になるため)。
  `seed` は自動で抜けるが、`click` などを直に叩くときは先に
  `click :: .welcome .btn` が要る
- **`seed` は旅一覧の画面にいるときだけ動く。** 旅の画面から呼ぶと
  「旅をつくる」ボタンが無くてタイムアウトする。先に `reset` するのが確実
- **`seed` の日程は今日を挟む4日固定**(昨日〜3日後)。タイトルに「2泊3日」と書いても
  日数は変わらない。**今日は必ず Day 2**になるので、現在時刻ラインを試せる
- **日付は `toISOString()` で作らない。** あれは UTC なので、深夜に走らせると
